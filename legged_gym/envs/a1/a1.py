...
class A1(LeggedRobot):
    ...
    def update_command_curriculum(self, env_ids):
        ...
        if torch.mean(self.episode_sums["lin_vel_tracking"][env_ids]) / self.max_episode_length > 0.75 * \
                self.reward_scales[
                    "lin_vel_tracking"]:
            self.command_ranges["lin_vel_x"][0] = np.clip(self.command_ranges["lin_vel_x"][0] - 0.25,
                                                          self.cfg.commands.min_curriculum_x, 0.5)
            self.command_ranges["lin_vel_x"][1] = np.clip(self.command_ranges["lin_vel_x"][1] + 0.25, 0.5,
        ...
        if torch.mean(self.episode_sums["ang_vel_tracking"][env_ids]) / self.max_episode_length > 0.75 * \
                self.reward_scales[
                    "ang_vel_tracking"]:
